...
use web_sys::{
    ...
    RtcDataChannelState, RtcIceServer, RtcPeerConnection, RtcPeerConnectionIceEvent,
...
impl PeerConnection {
    pub fn send_message(&self, message: &str) -> bool {
        self.dc
            ...
            .as_ref()
            .filter(|dc| {
                dc.ready_state() == RtcDataChannelState::Open && !message.trim().is_empty()
            })
            .map(|dc| {
...
pub fn create_host_peer_connection(callbacks: PeerConnectionCallbacks) -> PeerConnection {
    create_rtc_peer_connection()
        .pipe(|pc| (pc.create_data_channel("app"), pc))
        .pipe(|(dc, pc)| {
...
pub async fn create_guest_peer_connection(
    offer: &str,
    callbacks: PeerConnectionCallbacks,
) -> Result<PeerConnection, JsValue> {
    ...
    create_rtc_peer_connection()
        ...
        .pipe(|pc| async move {
            js_sys::JSON::parse(offer)?
                .pipe(|sdp| JsFuture::from(pc.set_remote_description(&sdp.into())))
                .await?;

            JsFuture::from(pc.create_answer())
                .await?
                .pipe(|answer| JsFuture::from(pc.set_local_description(&answer.into())))
                .await?;
...
fn create_rtc_peer_connection() -> RtcPeerConnection {
    RtcIceServer::new()
        .tap(|s| {
            s.set_urls(&js_sys::Array::of1(&JsValue::from_str(
                "stun:stun.l.google.com:19302",
            )))
        })
        .pipe(|ice_server| {
            RtcConfiguration::new().tap(|c| c.set_ice_servers(&js_sys::Array::of1(&ice_server)))
        })
...
fn setup_callbacks(
    ...
    pc.tap(|pc| setup_ice_callback(pc, callbacks.on_sdp_ready))
        .tap(|pc| setup_connection_state_callback(pc, callbacks.on_connection_status_change));
...
fn setup_guest_data_channel(
    pc: &RtcPeerConnection,
    dc_storage: Rc<RefCell<Option<RtcDataChannel>>>,
    on_open: SimpleCallback,
    on_message: StringCallback,
) -> Result<(), JsValue> {
    let callbacks = (
        Rc::new(RefCell::new(Some(on_open))),
        Rc::new(RefCell::new(Some(on_message))),
    );
    ...
    Closure::wrap({
        ...
        Box::new(move |e: RtcDataChannelEvent| {
            e.channel()
                ...
                .tap(|dc| {
                    (on_open.borrow_mut().take(), on_message.borrow_mut().take()).pipe(
                        |(open_cb, msg_cb)| match (open_cb, msg_cb) {
...
fn setup_data_channel_callbacks(
    dc: &RtcDataChannel,
    mut on_open: SimpleCallback,
    mut on_message: StringCallback,
) {
